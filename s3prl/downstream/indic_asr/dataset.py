...
class SequenceDataset(Dataset):
    ...
    def __init__(self, split, bucket_size, dictionary, data_root, bucket_file, **kwargs):
        ...
        for item in self.split_sets:
            file_path = os.path.join(bucket_file, item + "/bucket.csv")
        ...
        Y = [" ".join(list(x.strip().replace(" ", "|"))) + " |" for x in table_list['transcript'].tolist()]
        ...
        # Y = {key:trans for key,trans in zip(X,Y)}
    ...
    #         path = os.path.join(self.data_root, dir, trans_path)
