...
class SpeakerClassifiDataset(Dataset):
    def __init__(self, mode, file_path, meta_data, max_timestep=None):
        ...
        self.classes = sorted(list(set(x.split('/')[-2] for x in self.usage_list if "chunk_data" in x)))
    ...
    def build_label(self, train_path_list):
        y = []
        ind = -5
        if 'chunk_data' in train_path_list[0]:
            ind = -2
        for path in train_path_list:
            y.append(self.class_name2label[path.split('/')[ind]])
        return y
    ...
    def test_unknown(self):
        ...
        dataset = []
        print("search specified wav name for test unknown set")
        for string in tqdm.tqdm(self.usage_list):
            if 'test' in string and 'test_known' not in string:
                dataset.append(string)
    ...
    def test_unknown_noisy(self):
        fp = str(self.root).replace('dc_data_clean_wav','dc_data_noisy_wav')
        print(fp)
        usage_list = glob.glob(f"{fp}/**/*.wav",recursive=True)

        dataset = []
        print("search specified wav name for test unknown noisy set")
        for string in tqdm.tqdm(usage_list):
            if 'test' in string and 'test_known' not in string:
                dataset.append(string)
        print("finish searching test unknown set wav")
