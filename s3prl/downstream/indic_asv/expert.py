...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, downstream_expert, expdir, **kwargs):
        ...
        # dataset
        train_file_path = Path(self.datarc['file_path']) / "train" / "audio" 
        valid_file_path = Path(self.datarc['file_path']) / "valid" / "audio"
        test_file_path = Path(self.datarc['file_path']) / "test_known" / "audio"
        test_unk_file_path = Path(self.datarc['file_path']) / "test" / "audio"

        test_noisy_file_path = Path(self.datarc['file_path'].replace('dc_data_clean_wav','dc_data_noisy_wav')) / "test_known" / "audio"
        test_unk_noisy_file_path = Path(self.datarc['file_path'].replace('dc_data_clean_wav','dc_data_noisy_wav')) / "test" / "audio"
        train_config = {
            "vad_config": self.datarc['vad_config'],
            "file_path": [train_file_path],
            "key_list": [self.datarc['lang']],
        ...
        valid_config = {
            ...
            "meta_data": self.datarc['valid_meta_data']
        }        
        self.valid_dataset = SpeakerVerifi_test(**valid_config)
        ...
        test_unk_config = {
            "vad_config": self.datarc['vad_config'],
            "file_path": test_unk_file_path, 
            "meta_data": self.datarc['test_unk_meta_data']
        }
        self.test_unk_dataset = SpeakerVerifi_test(**test_unk_config)

        test_noisy_config = {
            "vad_config": self.datarc['vad_config'],
            "file_path": test_noisy_file_path, 
            "meta_data": self.datarc['test_meta_data'].replace('test_known_data','test_known_noisy_data')
        }
        self.test_noisy_dataset = SpeakerVerifi_test(**test_noisy_config)

        test_unk_noisy_config = {
            "vad_config": self.datarc['vad_config'],
            "file_path": test_unk_noisy_file_path, 
            "meta_data": self.datarc['test_unk_meta_data'].replace('test_data','test_noisy_data')
        }
        self.test_unk_noisy_dataset = SpeakerVerifi_test(**test_unk_noisy_config)
    ...
    def get_dataloader(self, mode):
        ...
        if mode == 'train':
            ...
            return self._get_eval_dataloader(self.test_unk_dataset)
        elif mode == 'test_known_noisy':
            return self._get_eval_dataloader(self.test_noisy_dataset)
        elif mode == 'test_unknown_noisy':
            return self._get_eval_dataloader(self.test_unk_noisy_dataset)
    ...
    def get_test_known_dataloader(self):
        return self._get_eval_dataloader(self.test_unk_dataset)
    ...
    def forward(self, mode, features, utter_idx, labels, records, **kwargs):
        ...
        Args:
            features:
                ...
                put in the validice assigned by command-line args
        ...
        # elif mode in ['valid', 'test_known','test_unknown']:
    ...
    def log_records(self, mode, records, logger, global_step, **kwargs):
        ...
        if mode == 'train':
            ...
            logger.add_scalar(f'indic_asv/{mode}-loss', loss, global_step=global_step)
            print(f'indic_asv/{mode}-loss: {loss}')

        else:
        # elif mode in ['valid', 'test_known','test_unknown']:
            eer, *others = self.eval_metric(np.array(records['labels']), np.array(records['scores']))
            logger.add_scalar(f'indic_asv/{mode}-EER', eer, global_step=global_step)
            print(f'indic_ascv/{mode}-EER: {eer}')

            if eer < self.best_score and mode == 'valid':
